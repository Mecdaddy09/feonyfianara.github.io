...
function loadModel(file,overideMaterial=null) {
  loader1.load( file, async function ( gltf ) {
    ...
    model.scale.set(.005*model.scale.x, .005*model.scale.y, .005 * model.scale.z)
...
function init() {
    ...
    for(let j=0;j<6;j++)
    {
      try {
        loader1.load( "grounds/ground_"+i+"_"+j+".glb", async function ( gltf ) {
          ...
          ground_chunk.scale.set(.005*ground_chunk.scale.x, .005*ground_chunk.scale.y, .005 * ground_chunk.scale.z)
  ...
  loader1.load( 'all_buildings.glb', async function ( gltf ) {
    ...
    buildings.scale.set(.005*buildings.scale.x, .005*buildings.scale.y, .005 * buildings.scale.z)
  ...
  loader1.load( 'roads.glb', async function ( gltf ) {
    ...
    roads = gltf.scene;
    roads.scale.set(.005*roads.scale.x, .005*roads.scale.y, .005 * roads.scale.z)
    roads.position.y -= 6;
    // wait until the model can be added to the scene without blocking due to shader compilation
  ...
  loader1.load( 'rivers.glb', async function ( gltf ) {
    ...
    rivers = gltf.scene;
    rivers.scale.set(.005*rivers.scale.x, .005*rivers.scale.y, .005 * rivers.scale.z)
    rivers.position.y -= 6;
    // wait until the model can be added to the scene without blocking due to shader compilation
...
function exit360Scene() {
  ...
  const loader = new GLTFLoader(loadingManager);
loader.load("/assets/Tany.glb", (gltf) => {
  const fianaModel = gltf.scene;
  ...
  loader.load("/assets/Lalana.glb", (gltf) => {
    const fianaModel1 = gltf.scene;
  
    fianaModel1.traverse((o) => {
      if (o.isMesh) o.material = newMaterial;
    });
  
    const boundingBox1 = new THREE.Box3().setFromObject(fianaModel1);
    const center1 = new THREE.Vector3();
    boundingBox1.getCenter(center1);
  
    fianaModel1.position.copy(fianaModel.position); // Copier la position de fianaModel
    fianaModel1.position.y += 2.5; // Ajouter 2.5 unités vers le haut
    
  
    scene.add(fianaModel1);
  });
  
  loader.load("/assets/Trano.glb", (gltf) => {
    const fianaModel2 = gltf.scene;
  
    fianaModel2.traverse((o) => {
      if (o.isMesh) o.material = newMaterial;
    });
  
    const boundingBox2 = new THREE.Box3().setFromObject(fianaModel2);
    const center2 = new THREE.Vector3();
    boundingBox2.getCenter(center2);
  
    fianaModel2.position.copy(fianaModel.position); // Copier la position de fianaModel
    fianaModel2.position.y += 3; 
    fianaModel2.position.y += 9; // Ajuster la position vers le haut
  
    scene.add(fianaModel2);
  });
  

  fianaModel.traverse((o) => {
    if (o.isMesh) o.material = newMaterial;
    // if (o.isMesh) o.material = customMaterial;
    
  });

  fianaModel.position.set(0, 0, 0);


  const boundingBox = new THREE.Box3().setFromObject(fianaModel);
  const center = new THREE.Vector3();
  boundingBox.getCenter(center);

  fianaModel.position.sub(center);

  scene.add(fianaModel);
  scene.add(fianaModel1);
  scene.add(fianaModel2);
