...
let points = [
  {
    ...
    position: {
      x: 2.7315549850463867,
      y: 0.050000000745057886,
      z: 0.040550846606492996,
    },
    texture: earth,
...
function loadGLBModel(id, modelPath, callback) {
...
function findPointByPosition(points, targetPosition) {
  return points.find(
    (point) =>
      point.position &&
      point.position.x === targetPosition.x &&
      point.position.y === targetPosition.y &&
      point.position.z === targetPosition.z
  );
}

function handleObjectClick(poi) {
  const foundPoint = findPointByPosition(points, poi);

  if (foundPoint) {
    console.log("Point trouvé :", foundPoint);

    if (!transitioning) {
      // transitioning = true;
      if (foundPoint.texture) {
        create360(foundPoint.texture);
        explorationAnimation();
        // console.log("tonga aty");

        // Désactivez les contrôles de la scène principale
        controls.enabled = true;
      } else {
        console.error(
          "La propriété 'texture' est manquante pour le point trouvé :",
          foundPoint
        );
      }
    }
  } else {
    console.log("Point non trouvé");
  }
}

function explorationAnimation() {
  gsap.to(controls360.target, {
    // Utilisez les contrôles de la scène 360
    x: -0.1,
    y: 1.3,
    z: 0.7,
    duration: 2,
...
function createPOIs() {
  points.forEach((poi) => {
    ...
    loadGLBModel(poi.id, poi.modelPath, (model) => {
      // Ajustez l'échelle du modèle
      ...
      // Ajoutez un écouteur d'événements de clic pour cet objet GLB
      model.onClick = () => {
        console.log("Clic détecté sur l'objet GLB :", poi.id);

        // Exécutez la fonction explorationAnimation uniquement lors du premier clic
        if (isFirstClick) {
          explorationAnimation();
          isFirstClick = false; // Mettez à jour le drapeau pour indiquer que le premier clic a eu lieu
        }
      };

      // Assurez-vous que l'objet est cliquable
      model.castShadow = true;
      model.receiveShadow = true;
      model.userData.enableClick = true;

      // Ajoutez l'objet GLB à la liste des objets cliquables
...
function create360(image = null) {
  ...
  myText.position.z = -4;
  myText.color = new THREE.Color(0x9ec3e9);

  myText.sync();
  console.log("Création de la scène 360 terminée.");
}

// Gestionnaire d'événements pour le clic sur le bouton Exit
// function handleExitButtonClick() {
//   console.log("Quitter la scène 360 en cours...");

//   // Désactivez les contrôles de la scène 360
//   controls360.enabled = false;

//   // Masquez la scène 360
//   scene360.visible = false;

//   // Réactivez les contrôles de la scène principale
//   controls.enabled = true;

//   // Nettoyez la scène 360 en supprimant tous les objets ajoutés
//   while (scene360.children.length > 0) {
//     const child = scene360.children[0];
//     scene360.remove(child);
//     child.dispose(); // Assurez-vous de disposer correctement des objets pour libérer la mémoire
//   }

//   console.log("Retour à la scène principale terminé.");
// }
...
document.getElementById("cont").addEventListener("mousedown", (event) => {
  ...
  if (hitted && hitted.info) {
    console.log("info ===>", hitted);
    handleObjectClick(hitted.position);
  } else {
    console.log(hitted);
  }
});
